...
from context_engineering_rag.generation import generate_answer
...
from context_engineering_rag.retriever import hybrid_retrieve
...
def run_query(
    ...
    sources = [_node_with_score_to_retrieved_node(nws) for nws in retrieved]
    ...
    if should_abstain(sources):
        return QueryResponse(
            answer="",
            sources=sources,
            metadata={**metadata, "abstained_reason": "similarity_below_threshold"},
            abstained=True,
        )

    gen_start = time.perf_counter()
    try:
        answer = generate_answer(query=query, context_nodes=sources)
